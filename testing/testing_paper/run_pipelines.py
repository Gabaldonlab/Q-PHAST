...
if operating_system=="windows":
	...
	CurDir = "%s%stesting%stesting_paper"%(C_qCAST_dir, sep, sep)

else: CurDir = "%s%stesting%stesting_paper"%(qCAST_dir, sep, sep)
...
# run images
run_cmd("%s %s --os %s --module analyze_images --output %s%simage_analysis --docker_image mikischikora/qcast:v0.1 --plate_layout %s --images %s%sraw_images --keep_tmp_files"%(python_exec, main_py, operating_system, outdir, sep, plate_layout, CurDir, sep))
